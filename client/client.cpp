...
{
    //represents quite a general error designating the fact that
    // the message received from the server is not a correct HTTP response message
    enum http_error_codes
    {
        invalid_response = 1
    };

    //representing an error category, which includes the invalid_response
...
//Because our HTTP client application is going to be asynchronous,
// the user of the client when initiating a request,
// will need to provide a pointer to a callback function,
// which will be invoked when the request completes.
// We need to define a type representing a pointer to such a callback function.
typedef void (*Callback)(const HTTPRequest &request,    //Which request has completed
                         const HTTPResponse &response,  //What is the response
                         const system::error_code &ec); //the error code designating the error that occurred

//representing a HTTP response message sent to the client as a response to the request
class HTTPResponse
{
...
//representing a HTTP request containing functionality that constructs the HTTP request message
// based on information provided by the class user,
// sends it to the server, and then receives and parses the HTTP response message.
class HTTPRequest
{
...
private:
    //a method that is used as a callback for an asynchronous DNS name resolution operation
    ...
    //a method used as a callback for an asynchronous connection operation,
    ...
    //is a callback, which is called after the request message is sent to the server
    ...
    //called when the first portion of the response message, namely, status line, is received from the server
    ...
    // which is called when the next portion of the response message—the response headers block—arrives from the server
    ...
    //is used as a callback, which is called after the response body arrives from the server
    ...
    // and its purpose is to call the callback provided by the HTTPRequest class' user to notify it about the completion of the request
...
int main()
{
    try
    {
        //creating an instance of the asio::io_service::work object
        // in order to make sure that threads running the event loop do not exit this loop
        ...
        request_one->set_uri("/server.html");
        ...
        request_two->set_uri("/client.html");
